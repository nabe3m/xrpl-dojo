--- 
title: Introduction to XRPL development in JavaScript
description: index page for introduction to XRPL development in JavaScript
sidebar: 
  label: Introduction
  order: 1
---order: 1

This is an introduction to XRPL using JavaScript!

Although this is intended for XRPL or JavaScript beginners, this is a very basic part of the lecture, so please try to understand it as you go along.

You will also need the following environment to run the contents in this section.

## Required tools

- Text editor ([VS Code is recommended](/editor/install/))
- [Node.js installation](/getting-started/node-js/)

## Goal

- To learn the basics of how to create a JavaScript XRPL project
- To understand the basic usage of `xrpl.js`, a client library for JavaScript
- Understand the concept of wallets and how they work
- To be able to connect to the testnet using code
- To be able to send transactions on the testnet using code